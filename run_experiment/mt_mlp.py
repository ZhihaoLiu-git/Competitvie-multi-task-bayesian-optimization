...
def select_dataset(name):
    if name == 'generate_classification':
        ...
        df = pd.read_csv(".\\experiment_data\\dataset\\eeg_eye_state\\eeg-eye-state_csv.csv", sep=",", header=None)
        X = df.iloc[:, :-1]
        y = df.iloc[:, -1]
...
def get_next_task(cr=1,  num_task=4, data=None, selected_tasks=None, gp_model=None, 
                  design_x=None, betat=[5, 2], betap=[2, 2], iteration=0, cr4_seed=0):
    assert cr < len(betat)
    design_list = []
...
trials = 30
budget = 80
data_name = 'wine'  #['generate_classification', 'breast_cancer', 'wine', 'digits', 'iris', 'eeg_eye_state']
result_flag = True
save_flag = True
save_flag2 = True
...
cmt = GetCmtData(tasks_list=[0.1, 0.3, 1, 3], dataset=select_dataset(data_name))
...
for cr in cr_list:
    ...
    for trial in range(trials):
        ...
        for iteration in range(budget):
            ...
            # print(gp.ICM.B.W)
            next_task, next_x, x_id = get_next_task(cr=cr, num_task=4, data=data,
